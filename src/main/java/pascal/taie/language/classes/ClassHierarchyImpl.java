...
//
//        // [调试] 打印字段解析过程
//        logger.debug("[调试] resolveField: 在类 {} 中查找字段 {} (类型: {})",
//                jclass.getName(), name, type);
//        logger.debug("[调试]   jclass.isPhantom() = {}", jclass.isPhantom());
//
...
//                logger.debug("[调试]   为 phantom 类 {} 创建了 phantom 字段 {}", jclass.getName(), name);
//            }
//            return field;
//        }
...
//        if (field != null) {
//            logger.debug("[调试]   在类 {} 中找到字段 {}", jclass.getName(), name);
//            return field;
//        }
...
//        JClass superClass = jclass.getSuperClass();
//        if (superClass != null) {
//            logger.debug("[调试]   在类 {} 中未找到字段 {}，递归查找父类 {}",
//                    jclass.getName(), name, superClass.getName());
//            return resolveField(superClass, name, type);
//        } else {
//            logger.warn("[调试]   类 {} 没有父类，字段 {} 解析失败", jclass.getName(), name);
//        }
//        // 5. Otherwise, field lookup fails.
//        // [修复] 当字段解析失败时，在声明类上创建 phantom 字段，而不是返回 null
//        // 这样可以避免 FieldResolutionFailedException
//        logger.info("[修复] 字段 {} 在类 {} 的继承链中未找到，创建 phantom 字段", name, jclass.getName());
//        field = new JField(jclass, name, Set.of(),
//                type, null, AnnotationHolder.emptyHolder());
//        jclass.addPhantomField(name, type, field);
//        return field;
